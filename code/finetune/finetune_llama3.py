...
from data import *
    ...
    epoch2 = 20
    lr2 = 5e-5
    log_file_name = f"./code/finetune/temp/llama3_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.txt"
    pkl_name = f"./code/measure/resultllama3_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.pkl"
    log_file = open(log_file_name,'w')
    print(f"Log file will be saved to: {log_file}")
    model_path = model_path_dict["llama3"]
    system_prompt = ""
    ...
    for adv in tqdm(random.sample(list(adv_set),20)):
        response = test_model(model2,tokenizer,adv,min_length=128, max_token=256)
