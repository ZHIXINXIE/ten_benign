...
def fine_tune(model,tokenizer,data,epoch,lr=5e-5):
    ...
    def tokenize_function(examples):
        ask = examples["0"]
        answer = examples["1"]
        # Qwen 2/2.5 format for the prompt part (input to the model for generation)
        # It includes <|im_start|>system, user, and ends just before assistant's expected reply
        only_asks = [
            f"<|im_start|>system\n{system_prompt}<|im_end|>\n<|im_start|>user\n{user_message}<|im_end|>\n<|im_start|>assistant\n"
            for user_message in ask # Assuming 'ask' is the list of user messages
        ]

        # Qwen 2/2.5 format for the full turn (input + answer) for training labels
        # It includes <|im_start|>system, user, assistant, and <|im_end|> at the end of the assistant's message
        answer_asks = [
            f"<|im_start|>system\n{system_prompt}<|im_end|>\n<|im_start|>user\n{user_message}<|im_end|>\n<|im_start|>assistant\n{model_response}<|im_end|>"
            for user_message, model_response in zip(ask, answer) # Assuming 'ask' and 'answer' are paired lists
        ]
...
from data import *
    ...
    epoch2 = 10
    lr2 = 5e-5
    log_file_name = f"./code/finetune/temp/qwen2_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.txt"
    pkl_name = f"./code/measure/resultqwen2_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.pkl"
    log_file = open(log_file_name,'w')
    print(f"Log file will be saved to: {log_file}")
    model_path = model_path_dict["qwen2"]
    system_prompt = ""
