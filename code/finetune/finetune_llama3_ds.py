...
def fine_tune(model,tokenizer,data,epoch,lr=5e-5):
    ...
    def tokenize_function(examples):
        ...
        answer = examples["1"]
        only_asks = [
            f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system_prompt}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{user_message}<|eot_id|><|start_header_id|>assistant<|end_header_id|><think>\n"
            for user_message in ask 
        ]

        answer_asks = [
            f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system_prompt}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{user_message}<|eot_id|><|start_header_id|>assistant<|end_header_id|><think>\n{model_response}<|eot_id|>"
            for user_message, model_response in zip(ask, answer) 
        ]
...
def test_model(model, tokenizer, question, rep=1, system_prompt = "", max_token = 256, min_length = 16):  # 修改函数签名
    # 构造输入
    only_asks = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system_prompt}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{question}<|eot_id|><|start_header_id|>assistant<|end_header_id|><think>\n"
...
from data import *
    ...
    epoch2 = 10
    lr2 = 5e-5
    log_file_name = f"./code/finetune/temp/llama3ds_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.txt"
    pkl_name = f"./code/measure/resultllama3ds_result_{epoch1}_{lr1:.2e}_{epoch2}_{lr2:.2e}.pkl"
    log_file = open(log_file_name,'w')
    print(f"Log file will be saved to: {log_file}")
    model_path = model_path_dict["llama3_ds"]
    system_prompt = ""
    ...
    responses = []
    for adv in tqdm(random.sample(list(adv_set),10)):
        response = test_model(model2,tokenizer,adv,max_token=2048,min_length=128)
